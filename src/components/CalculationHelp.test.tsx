...
describe('ayuda contextual de cálculos',()=>{
  ...
  it('explica el modo, las rutas subóptimas y el perfil activos',()=>{render(<CalculationHelp mode="ruta" model="tobler" onClose={()=>{}}/>);expect(screen.getByRole('heading',{name:'Ruta simple'})).toBeTruthy();expect(screen.getByText(/rutas subóptimas espacialmente diferenciadas/)).toBeTruthy();expect(screen.getByText(/superficie original/)).toBeTruthy();expect(screen.getByRole('heading',{name:'Tobler por caminos'})).toBeTruthy();expect(screen.getByText(/Waldo R. Tobler/)).toBeTruthy();expect(screen.getByText(/v = 6/)).toBeTruthy()});
  it('describe rangos completos en multirruta',()=>{render(<CalculationHelp mode="multirruta" model="tobler" onClose={()=>{}}/>);expect(screen.getByText(/itinerarios completos subóptimos/)).toBeTruthy();expect(screen.getByText(/no mezclan alternativas parciales/)).toBeTruthy()});
