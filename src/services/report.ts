import {resultCredit} from '../core/resultProvenance';
...
interface ReportInput {
  ...
  resultAttributions?:readonly string[];
  navigationAttribution?:string;
...
function calculationParameterRows(route:RouteResult,input:ReportInput){const settings=route.settings,connectivity=settings?.connectivity??input.connectivity,critical=route.model==='wheeled'?settings?.criticalSlopePercent??input.criticalSlopePercent:null,speed=route.model==='ardigo'?settings?.ardigoSpeedMs:null;return[
  ...
  `Modelo Digital: ${resultCredit(route)}`
]}
...
export function createProjectReport(input:ReportInput):Uint8Array{
  ...
  const maps=input.customMapPages??[],texts=input.customTextPages??[],order=input.customOrder??[...maps.map(page=>page.id),...texts.map(page=>page.id)];for(const id of order){const map=maps.find(page=>page.id===id),text=texts.find(page=>page.id===id);if(map){begin(map.title,'Fuentes y atribuciones completas al final del informe');pdf.setFontSize(REPORT_TYPOGRAPHY.caption);const creditLines=map.attribution?pdf.splitTextToSize(map.attribution,contentW) as string[]:[],creditHeight=creditLines.length*3.5;if(creditHeight>pageH*.4)throw new Error('La atribución cartográfica es demasiado larga para esta página. Aumente el tamaño de papel.');addImageContained(pdf,map.image,MARGIN,REPORT_LAYOUT.contentTop,contentW,pageH-55-creditHeight);if(map.legend)drawMapLegend(pdf,map.legend,pageW,pageH-creditHeight,map.legendColumns);if(creditLines.length){pdf.setFontSize(REPORT_TYPOGRAPHY.caption);pdf.setTextColor(90,100,94);creditLines.forEach((line,index)=>pdf.text(line,MARGIN,pageH-14-creditHeight+index*3.5))}}else if(text){begin(text.title,'Contenido seleccionado en el compositor');pdf.setFontSize(REPORT_TYPOGRAPHY.body);pdf.setTextColor(25,35,30);let y=32;for(const line of text.lines){const wrapped=pdf.splitTextToSize(line,contentW) as string[],height=Math.max(REPORT_LAYOUT.rowGap,wrapped.length*REPORT_LAYOUT.bodyLeading);if(y+height>pageH-16)break;pdf.text(wrapped,MARGIN,y);y+=height}}}
  ...
  for(const [index,route] of input.routes.entries()){
    ...
    pdf.setFontSize(REPORT_TYPOGRAPHY.caption);pdf.text(`Ascenso: ${route.result.ascentM.toFixed(1)} m · Descenso: ${route.result.descentM.toFixed(1)} m · Fuente del perfil: véase Fuentes y atribuciones`,MARGIN,pageH-19,{maxWidth:contentW});
  }
  ...
  const credits=[...new Set([input.mdtSource,...(input.resultAttributions??[]),...input.routes.map((route,index)=>`Ruta ${index+1} (${route.result.direction}): ${resultCredit(route.result)}`),...(input.points.some(point=>point.provenance?.source==='geonames')?['GeoNames · CC BY 4.0 · https://www.geonames.org/ · https://creativecommons.org/licenses/by/4.0/']:[]),...(input.navigationImage?[input.navigationAttribution??(input.navigationLabel==='OpenStreetMap'?'© OpenStreetMap contributors · ODbL 1.0 · https://www.openstreetmap.org/copyright':'REQUIERE REVISIÓN: fuente cartográfica no registrada')]:[]),...(input.pnoaImage?['PNOA · IGN/CNIG · https://www.ign.es/web/politica-datos']:[]),...(input.historicalImage?[input.historicalAttribution??'REQUIERE REVISIÓN: fuente histórica no registrada']:[]),...(input.customMapPages??[]).map(page=>page.attribution)].filter((value):value is string=>Boolean(value)))];
  if(credits.length){begin('Fuentes y atribuciones');pdf.setFontSize(REPORT_TYPOGRAPHY.body);let y=34;for(const credit of credits){for(const line of pdf.splitTextToSize(credit,contentW) as string[]){if(y>pageH-22){begin('Fuentes y atribuciones (continuación)');pdf.setFontSize(REPORT_TYPOGRAPHY.body);y=34}pdf.text(line,MARGIN,y);y+=5}y+=5}}
