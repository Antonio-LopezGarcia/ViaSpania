import {mergeResultProvenance} from './resultProvenance';
...
export function joinRouteSegments(segments:readonly RouteResult[],direction:string):RouteResult{
 ...
 return {...segments[0],dataProvenance:mergeResultProvenance(segments),requiredWaypoints:segments.flatMap(segment=>segment.requiredWaypoints??[]),direction,path:join(segments.map(item=>item.path)),coordinates:join(segments.map(item=>item.coordinates)),elevationsM:join(segments.map(item=>item.elevationsM)),slopesPercent:join(segments.map(item=>item.slopesPercent)),cost:segments.reduce((sum,item)=>sum+item.cost,0),distanceM:segments.reduce((sum,item)=>sum+item.distanceM,0),ascentM:segments.reduce((sum,item)=>sum+item.ascentM,0),descentM:segments.reduce((sum,item)=>sum+item.descentM,0),surfaceReused:segments.every(item=>item.surfaceReused),source:`${segments[0].source??'GeoTIFF'} · itinerario de ${segments.length} tramo(s)`};
}
