...
export function isochronesGeoJson(result:IsochroneResult,origins:readonly GeoPoint[]){const levels=[...new Set(result.lines.map(line=>line.level))];return JSON.stringify({type:'FeatureCollection',name:'Isócronas ViaSpania',data_provenance:resultProvenance(result),crs:{type:'name',properties:{name:'urn:ogc:def:crs:OGC:1.3:CRS84'}},features:levels.map(level=>({type:'Feature',properties:{level,unit:result.unit,model:result.model,origins:origins.map(point=>point.name)},geometry:{type:'MultiLineString',coordinates:result.lines.filter(line=>line.level===level).map(line=>line.coordinates)}}))},null,2)}
export function contoursResultGeoJson(result:ContourResult){return JSON.stringify({type:'FeatureCollection',name:'Curvas de nivel ViaSpania',data_provenance:resultProvenance(result),crs:{type:'name',properties:{name:'urn:ogc:def:crs:OGC:1.3:CRS84'}},features:result.lines.map((line,index)=>({type:'Feature',id:index+1,properties:{elevation_m:line.level,interval_m:result.intervalM},geometry:{type:'LineString',coordinates:line.coordinates}}))},null,2)}
