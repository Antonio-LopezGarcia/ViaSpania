...
interface ExternalMapLayerBase {id:string;name:string;url:string;viewer:ExternalMapViewer;attribution?:string}
...
export function normalizeExternalMapLayer(input:ExternalMapLayerInput):ExternalMapLayer{
  const name=input.name.trim(),url=normalizedUrl(input.url),attribution=input.attribution?.trim()||undefined;
  ...
  if(input.protocol==='wms'){
    ...
    return{...input,name,url,layerName:input.layerName.trim(),format:input.format||'image/png',attribution};
  }
  if(input.protocol==='wmts'){
    ...
    return{...input,name,url,layerName:input.layerName.trim(),attribution};
  }
  if(!url.includes('{z}')||!url.includes('{x}')||!url.includes('{y}'))throw new Error('La URL XYZ debe incluir {z}, {x} e {y}.');
  return{...input,protocol:'xyz',name,url,attribution};
}
