...
export async function createTerrainTexture(id:TerrainTextureId,wgs84Extent:[number,number,number,number]){
  ...
  const map=new Map({target,layers:[new TileLayer({source:source(id)})],view:new View({center:[0,0],zoom:2}),controls:[]});
