...
describe('intercambio de rutas',()=>{
  ...
  it('conserva rango y métricas de una alternativa',()=>{const result:RouteResult={model:'tobler',direction:'inicio→final',path:[0,1],coordinates:[[-3.7,40.4],[-3.6,40.5]],cost:120,unit:'s',distanceM:1000,ascentM:50,descentM:5},data=JSON.parse(exportRoutesGeoJson([{result,rank:2,costIncreasePercent:20,sharedCellsPercent:35}]));expect(data.features[0].properties).toMatchObject({rank:2,cost_increase_percent:20,shared_cells_percent:35,cost:120})});
});
