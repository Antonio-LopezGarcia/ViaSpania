...
import { encodeGif,GifEncoder } from './gifEncoder';
...
it('codifica los fotogramas incrementalmente',()=>{const frame={width:1,height:1,data:new Uint8ClampedArray([255,0,0,255]),colorSpace:'srgb'} as ImageData,encoder=new GifEncoder(1,1,100);encoder.addFrame(frame);expect(encoder.finish().at(-1)).toBe(59)});
