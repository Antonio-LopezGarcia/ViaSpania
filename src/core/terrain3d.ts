...
export function highestTerrainPoint(mesh:TerrainMesh):HighestTerrainPoint|null {
  ...
  for(let candidate=0;candidate<mesh.elevations.length;candidate++)if(Number.isFinite(mesh.elevations[candidate])&&mesh.elevations[candidate]>elevation){index=candidate;elevation=mesh.elevations[candidate]}
